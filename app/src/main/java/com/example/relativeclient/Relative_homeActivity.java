...
import com.example.relativeclient.Mythread.MythreadGetgps;
...
public class Relative_homeActivity extends Activity{
    GeocodeSearch geocoderSearch;
    Button button_home, button_home_bind, button_information,
            button_call_v, button_call_v_record;
    FloatingActionButton button_refresh;
    TextView clue_pic, blind_onlinestatuse,blind_loc;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        //申请通知权限
        if (ContextCompat.checkSelfPermission(Relative_homeActivity.this,
                Manifest.permission.POST_NOTIFICATIONS) != PackageManager.PERMISSION_GRANTED) {
            ActivityCompat.requestPermissions(Relative_homeActivity.this,
...
//        }
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            ...
            // 添加 FLAG_DRAWS_SYSTEM_BAR_BACKGROUNDS 标志
            window.addFlags(WindowManager.LayoutParams.FLAG_DRAWS_SYSTEM_BAR_BACKGROUNDS);
            // 设置状态栏颜色为透明
        ...
        setContentView(R.layout.relative_home);
        {
            ...
            button_refresh = findViewById(R.id.button_refresh1);
//            blind_loc=findViewById(R.id.blind_loc);
...
//            aMap.moveCamera(CameraUpdateFactory.newLatLngZoom(latLng,16));
            ...
            button_refresh.setOnClickListener(new OnClickListener() {
                ...
                public void onClick(View v) {
                    ...
                    //首先判断是否在线
                    button_refresh.animate().rotationBy(360).setDuration(500).start(); // 转圈动画
                    if (!getgps) {
                        Toast.makeText(getApplicationContext(), "当前盲人离线中，无法获取其位置信息",
                                Toast.LENGTH_LONG).show();
                        finish();
                        //刷新当前活动
                        Intent intent = new Intent(Relative_homeActivity.this, Relative_homeActivity.class);
                        startActivity(intent);
                    }
                    //打开地图
                    else {
                        ...
                        //'''更新点坐标'''
                        MythreadGetgps t=new MythreadGetgps(User.getBlindId(),Relative_homeActivity.this);
                        t.start();
                        try {
                            t.join();
                        ...
                        System.out.println(User.getBlindLat()+User.getBlindLon());
                        LatLng desLatLng;
                        if(convert_pos){
                            //坐标转化
                            CoordinateConverter converter  = new CoordinateConverter(Relative_homeActivity.this);
                        ...
                        //画点
                        // 定义图标资源（假设你的图标文件名为 marker_icon.png）
                        // 清除之前的标记点
...
//                        BitmapDescriptor markerIcon = BitmapDescriptorFactory.fromResource(R.mipmap.locationpurple2);
//
//                        Marker marker = aMap.addMarker(new MarkerOptions()
//                                .position(desLatLng)
//                                .title("您守护的视障人士的位置")
//                                .snippet(User.getBlindLoc())
//                                .icon(markerIcon) // 设置自定义图标
//                                .anchor(0.5f, 0.5f) // 可选：设置图标锚点为中心（默认底部中心）
//                        );
//
//// 移动视角到标记点
//                        aMap.moveCamera(CameraUpdateFactory.newLatLngZoom(desLatLng, 16));
////                        final Marker marker = aMap.addMarker(new MarkerOptions().position(desLatLng).title("您守护的视障人士的位置").snippet(User.getBlindLoc()));
////                        aMap.moveCamera(CameraUpdateFactory.newLatLngZoom(desLatLng,16));

                        // 创建新标记点（保持原有逻辑）
                        BitmapDescriptor markerIcon = BitmapDescriptorFactory.fromResource(R.mipmap.locationpurple2);
                        previousMarker = aMap.addMarker(new MarkerOptions()
                                .position(desLatLng)
                                .title("您守护的视障人士的位置")
                                .snippet(User.getBlindLoc())
                                .icon(markerIcon)
            ...
            button_home_bind.setOnClickListener(new OnClickListener() {
                ...
                public void onClick(View view) {
                    ...
                    if (User.getBlindId() != -1) {
                        Toast.makeText(Relative_homeActivity.this, "您已绑定过账号，如要修改请转至个人信息界面", Toast.LENGTH_SHORT).show();
                    } else {
                        Intent intent = new Intent(Relative_homeActivity.this, BindingActivity.class);
                        startActivity(intent);
                    }
                }
            });
            ...
            button_information.setOnClickListener(new OnClickListener() {
                @Override
                public void onClick(View view) {
                    Intent intent = new Intent(Relative_homeActivity.this, InformationActivity.class);
                    startActivity(intent);
                }
            });
            ...
            button_call_v.setOnClickListener(new OnClickListener() {
                ...
                public void onClick(View view) {
                    ...
                    {
                        Intent intent = new Intent(Relative_homeActivity.this, PhoneActivity.class);
                        startActivity(intent);
                    } else if (User.getBlindState() == 3) {//在线但未被帮助
                        Toast.makeText(Relative_homeActivity.this,
            ...
            button_call_v_record.setOnClickListener(new OnClickListener() {
                ...
                public void onClick(View view) {
                    ...
                    MythreadGetRecodrd t=new MythreadGetRecodrd(blind_id,Relative_homeActivity.this);
                    t.start();
                    try {
                        t.join();
                    } catch (InterruptedException e) {
                    ...
                    if (activities.isAc_exist() == true) {
                        Intent intent = new Intent(Relative_homeActivity.this, Helped_historyActivity.class);
                        startActivity(intent);
                    } else {
                        Toast.makeText(Relative_homeActivity.this,
...
//            if (msg.what == 0) {
//                Toast.makeText(Relative_homeActivity.this, "请求失败，请检查网络或稍后重试", Toast.LENGTH_LONG).show();
//            }
//        }
//    };
            ...
            public void run() {
                ...
                //获得对象
                {
                    ...
                    if (user != null) {
                        ...
                        if (history_count >= 30)//以每20s收集一次的话
                        {
                            //要发信息
                            setNotification();
                            //发完信息
                            history_count -=9 ;//设置过3分钟又来发信息
    ...
    private void convertLocation(double lon, double lat,
                                 final Relative_homeActivity.OnLocationConvertListener listener) {
    ...
    private void setNotification() {
        if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.O) {
            ...
            Intent intent = new Intent(this, Relative_homeActivity.class);
    ...
    private void resetOtherButtons() {
        button_home.setBackgroundColor(Color.GRAY);
        button_call_v.setBackgroundColor(Color.GRAY);
        button_information.setBackgroundColor(Color.BLUE);
        button_home_bind.setBackgroundColor(Color.GRAY);
        button_call_v_record.setBackgroundColor(Color.GRAY);
    }
